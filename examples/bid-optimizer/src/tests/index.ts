...
describe("Test Example BidOptimizer", function() {
  ...
  const bidOptimizer: core.ResponseData<core.BidOptimizer> = {
    status: "ok",
    data: {
      id: "1000",
      name: "my analyzer",
  ...
  rpMockup
    .withArgs(
      sinon.match.has(
        ...
        sinon.match(function(value: string) {
          return (
            value.match(/\/v1\/bid_optimizers\/(.){1,10}\/properties/) !== null
          );
        })
      )
    )
          ...
          .end((err, res) => {
            ...
            request(runner.plugin.app)
              ...
              .end((err, res) => {
                ...
                expect(
                  (JSON.parse(res.text) as core.BidOptimizerPluginResponse)
                    .bids[0].bid_price
                ).to.be.eq(bidDecisionRequest.campaign_info.max_bid_price);
                done();
              });
          });
      });
