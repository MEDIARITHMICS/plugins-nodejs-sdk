...
export class BasePlugin {
         ...
         onLogLevelRequest(req: express.Request, res: express.Response) {
         ...
         requestGatewayHelper(method: string, uri: string, body?: any) {
           const options = { method: method, uri: uri, json: true, auth: { user: this.worker_id, pass: this.authentication_token, sendImmediately: true } };

           return this._transport(body ? Object.assign({ body: body }, options) : options).catch(
             function(e: any) {
         ...
         constructor() {
           ...
           this.server = this.app.listen(this.pluginPort, () =>
             this.logger.info(
               "Plugin started, listening at " + this.pluginPort
             )
           );
         }
       }
