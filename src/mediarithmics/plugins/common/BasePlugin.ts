...
export class PropertiesWrapper {
  ...
  findAssetFileProperty = (key?: string): Option<AssetFileProperty> => {
    const p = key
      ? this.get(key)
      : this.ofType("ASSET") || this.ofType("ASSET_FILE");
...
export abstract class BasePlugin<CacheValue = any> {
  ...
  async requestGatewayHelper(
    ...
    try {
      ...
      if (e.name === "StatusCodeError") {
        ...
        throw new Error(
          `Error while calling ${method} '${uri}' with the request body '${
            bodyString || ""
          }', the qs '${JSON.stringify(qs) || ""}', the auth user '${
            obfuscateString(options.auth ? options.auth.user : undefined) || ""
          }', the auth password '${
            obfuscateString(options.auth ? options.auth.pass : undefined) || ""
  ...
  async requestPublicMicsApiHelper(
    apiToken: string,
      ...
      if (e.name === "StatusCodeError") {
        throw new Error(
          `Error while calling ${options.method} '${
            options.uri
          }' with the header body '${JSON.stringify(options.headers)}': got a ${
  ...
  async fetchDatamarts(
    apiToken: string,
    organisationId: string
  ): Promise<DataListResponse<Datamart>> {
    const options = {
      method: "GET",
  ...
  async fetchDatamartCompartments(
    apiToken: string,
    datamartId: string
  ): Promise<DataListResponse<Compartment>> {
    const options = {
      method: "GET",
